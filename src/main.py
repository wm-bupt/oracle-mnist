...
class ImageList(Dataset):
    ...
    def __init__(self, path, kind, transform=None):
        (train_set, train_labels) = mnist_reader.load_mnist(path, kind)
...
def main():
    ...
    parser.add_argument('--test-batch-size', type=int, default=300, metavar='N', help='input batch size for testing (default: 1000)')
    ...
    train_data = ImageList(path='../data/oracle/', kind='train',
    ...
    test_data = ImageList(path='../data/oracle/', kind='t10k',
                          transform=transforms.Compose([
                              transforms.ToTensor(),
                              transforms.Normalize((0.5,), (0.5,))
                          ]))
